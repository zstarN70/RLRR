...
class RLRRLinear(nn.Linear):
    def __init__(self, size_in, size_out, bias=True, rlrr_enable=False):
        super(RLRRLinear, self).__init__(size_in, size_out, bias=True)
        self.rlrr_enable = rlrr_enable
    ...
    def forward(self, x):
        if self.rlrr_enable:
            weight = (self.weight * self.scale_line * self.scale_col) + self.weight
...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, bias=True, drop=0.,
        ...
        self.fc1 = RLRRLinear(in_features, hidden_features, bias=bias[0], rlrr_enable=rlrr_enable)
        self.act = act_layer()
        self.drop1 = nn.Dropout(drop_probs[0])
        self.fc2 = RLRRLinear(hidden_features, out_features, bias=bias[1], rlrr_enable=rlrr_enable)
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, attn_drop=0., proj_drop=0., rlrr_enable=True):
        ...
        self.qkv = RLRRLinear(dim, dim * 3, bias=qkv_bias, rlrr_enable=rlrr_enable)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = RLRRLinear(dim, dim, rlrr_enable=rlrr_enable)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, rlrr_enable=True):
        super().__init__()
        self.dim = dim
        ...
        self.attn = WindowAttention(
            ...
            attn_drop=attn_drop, proj_drop=drop, rlrr_enable=rlrr_enable)

        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop,
                       rlrr_enable=rlrr_enable)
        ...
        self.rlrr_enable = rlrr_enable
        if self.rlrr_enable:
            self.ssf_scale_norm1, self.ssf_shift_norm1 = init_ssf_scale_shift(dim)
            self.ssf_scale_norm2, self.ssf_shift_norm2 = init_ssf_scale_shift(dim)

        self.register_buffer("attn_mask", attn_mask)

    def ARC_adapter(self, x, u, adapter_drop, lambda_, bias):
        hidden_states = x

        attention_uv_output = torch.matmul(hidden_states, u * lambda_)
        attention_uv_output = adapter_drop(attention_uv_output)
        attention_uv_output = torch.matmul(attention_uv_output, u.t()) + bias
        attention_uv_output = hidden_states + attention_uv_output

        return attention_uv_output
    ...
    def forward(self, x):
        ...
        x = self.norm1(x)
        if self.rlrr_enable:
            x = ssf_ada(x, self.ssf_scale_norm1, self.ssf_shift_norm1)
        ...
        x = x + self.drop_path(self.mlp(ssf_ada(self.norm2(x), self.ssf_scale_norm2, self.ssf_shift_norm2)))
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm, rlrr_enable=True):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
        self.reduction = RLRRLinear(4 * dim, 2 * dim, bias=False, rlrr_enable=rlrr_enable)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False, rlrr_enable=True):
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(
                ...
                window_size=window_size, shift_size=0 if (i % 2 == 0) else window_size // 2, mlp_ratio=mlp_ratio,
        ...
        if downsample is not None:
            self.downsample = downsample(input_resolution, dim=dim, norm_layer=norm_layer, rlrr_enable=rlrr_enable)
        else:
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 embed_dim=96, depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24), svd_dim=50,
                 window_size=7, mlp_ratio=4., qkv_bias=True,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
                 use_checkpoint=False, weight_init='', rlrr_enable=True, **kwargs):
        super().__init__()
        ...
        self.rlrr_enable = rlrr_enable
        if self.rlrr_enable:
            self.ssf_scale_encode_norm, self.ssf_shift_encode_norm = init_ssf_scale_shift(self.num_features)
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x)
        if self.rlrr_enable:
            x = ssf_ada(x, self.ssf_scale_encode_norm, self.ssf_shift_encode_norm)
...
if __name__ == '__main__':
    models = swin_base_patch4_window7_224_in22k(pretrained=True, num_classes=10, drop_rate=0.1)
